...
class SimpleAgent(base_agent.BaseAgent): 
    ...
    """
    Sélectionne quelque chose en fonction des coordonnées
    """
    def select_building(self, obs, coord):
      if _SELECT_POINT in obs.observation["available_actions"]:
        self.factory_selected = True
        return actions.FUNCTIONS.select_point("select", coord)
    ...
    def train_combat_unit_in_factory(self, obs, combat_unit, coord):
      if self.factory_selected == False:
        return self.select_building(obs, coord)
      else:
    ...
    def step(self, obs):
      ...
      if value_structures == 1175 and food_army <= 8: # Une usine coûte 250 de minerai ???
        self.scv_selected = False
        return self.train_combat_unit_in_factory(obs, "hellion", self.change_location(obs, 27, 2))

      if value_structures == 1175 and food_army >= 1:
        if self.scv_selected == False:
          return self.select_scv(obs)
        else:
          self.factory_selected = False
      ...
      if value_structures == 1525: # Une armurerie coûte 250 de minerai
        if self.factory_selected == False:
          self.factory_selected = True
          return self.select_building(obs, self.change_location(obs, 27, 2))
        if self.techLab_built == False and self.factory_selected == True:
          self.scv_selected = False
          return self.build_techLab(obs)

      if value_structures == 1600:
        print("a")
        if self.scv_selected == False:
          print("b")
          return self.select_scv(obs)
          ...
          print("c")
          self.factory_selected = False
        return self.build_factory(obs, self.change_location(obs, 30, -13))

      if value_structures >= 1825:
        if food_army > 16:
          self.factory_selected = False
          self.barracks_selected = False
          return self.attack(obs)
          ...
          self.state_attack = 0
          if self.factory_0_selected == True:
            self.factory_0_selected = False
            if (collected_vespene - spent_vespene) > 200:
              return self.train_combat_unit_in_factory(obs, "thor", self.change_location(obs, 27, 2))
            return self.train_combat_unit_in_factory(obs, "hellion", self.change_location(obs, 27, 2))
          else:
            self.factory_0_selected = True
            return self.train_combat_unit_in_factory(obs, "hellion", self.change_location(obs, 30, -13))
